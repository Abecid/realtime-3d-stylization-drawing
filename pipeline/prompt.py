...
# return the directional text prompt to use depending on the view azim angle
def azim_text_prompt(text, pose):
	elev, azim = pose
	if elev > 30:
		return f"{text}, top view."
	elif elev < -30:
		return f"{text}, bottom view."
	else:
		if azim > 180:
			azim -= 360
		if azim >= -30 and azim <= 30:
			return f"{text}, front view."
		elif azim <=-120 or azim >= 120:
			return f"{text}, back view."
		else:
			return f"{text}, side view."


# return the opposite directional text prompt for negative prompt
@torch.no_grad()
def azim_neg_text_prompt(text, pose):
	elev, azim = pose
	if azim > 180:
		azim -= 360
	if azim > -30 and azim < 30:
